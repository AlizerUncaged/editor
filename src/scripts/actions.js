...
const defaultLevel =
  "kS38,1_40_2_125_3_255_4_-1_6_1000_7_1|1_0_2_102_3_255_4_-1_6_1001_7_1|1_0_2_102_3_255_4_-1_6_1009_7_1|1_255_2_255_3_255_4_-1_6_1004_7_1|1_255_2_255_3_255_4_-1_6_1002_7_1|,kA13,0,kA15,0,kA16,0,kA14,,kA6,1,kA7,1,kA17,1,kA18,0,kS39,0,kA2,0,kA3,0,kA8,0,kA4,0,kA9,0,kA10,0,kA11,0";
let gdPath = "";
if (window.process)
  gdPath =
    window.process.env.LOCALAPPDATA + "\\GeometryDash\\CCLocalLevels.dat";
if (localStorage.getItem("settings.gdLevelsPath"))
  gdPath = localStorage.getItem("settings.gdLevelsPath");
  ...
  switch (action) {
          ...
          if (fs) {
            if (!checkGDExists()) {
              util.alert(
                "loadDialog",
                "Couldn't load!",
                "Geometry Dash is not detected on your computer! Make sure you install GD before saving/loading levels.",
                "OK"
              );
              return;
            }
            ...
            ccll.forEach((l) => {
              ...
              let description = `♫ ${song} | ${lengths[l.length || 0]} | ${
            ...
            util.createDialog("loadDialog", "Select a Level to Load", true, [
              obj,
            ]);
          } else
            util.alert(
              "loadDialog",
              "Couldn't load!",
              "Reading files is only supported on GDExt desktop!",
              "OK"
            );
        }
      });
    ...
    case "save":
      ...
      if (window.gdext && window.gdext.isGdRunning) {
        util.alert(
          "saveDialog",
          "Couldn't save!",
          "Geometry Dash is currently running! Please close it and try again.",
          "OK"
        );
        return;
      }
      ...
      if (fs) {
        ...
        if (localStorage.getItem("lvlnumber") == -1) {
          ...
          ccll[parseInt(localStorage.getItem("lvlnumber"))].name =
            localStorage.getItem("lvlname");
          ccll[parseInt(localStorage.getItem("lvlnumber"))].song =
            localStorage.getItem("lvlsong");
        }
        ...
        if (autosaveTimeLabel)
          autosaveTimeLabel.innerText = "Last saved just now";
      } else
        util.alert(
          "saveDialog",
          "Couldn't save!",
          "Saving is only supported on GDExt desktop!",
          "OK"
        );
      break;
    case "saveas":
      ...
      if (fs) {
        ...
        util.prompt(
          "saveasDialog",
          "New Level Name",
          "Enter the new name of the copied level:",
          {
            defaultValue: () => {
              return localStorage.getItem("lvlname");
            },
            maxlength: 20,
            onConfirm: (v) => {
              if (!v) return;
              ...
              dateSaved = new Date();
              let autosaveTimeLabel =
                document.querySelector("#autosaveTimeLabel");
              if (autosaveTimeLabel)
                autosaveTimeLabel.innerText = "Last saved just now";
            },
          }
    ...
    case "renameLevel":
      util.prompt(
        "renameDialog",
        "Rename Level",
        "Enter the new name of the level:",
        {
          defaultValue: () => {
            return localStorage.getItem("lvlname");
          },
...
export default {
  init: () => {
    ...
    window.addEventListener("gdRunningStateChange", (e) => {
      ...
      if (localStorage.getItem(`gdRunningAlertDontShowAgain`) != "true") {
        if (e.detail) {
          util.alert(
            "gdRunningAlert",
            "Looks Like You've Opened GD!",
            "For stability reasons, you won't be able to save levels in GDExt until you close Geometry Dash",
            "Got It",
            true
          );
        } else {
          if (document.querySelector("#gdRunningAlert"))
            util.closeDialog("gdRunningAlert");
        }
      }
    });
    ...
    let lastSavedTextInterval = setInterval(() => {
      let autosaveTimeLabel = document.querySelector("#autosaveTimeLabel");
      if (autosaveTimeLabel)
        autosaveTimeLabel.innerText =
          "Last saved " +
          (dateSaved ? util.getTimeDifferenceText(dateSaved) : "a while back");
    }, 60000);

    //check if level has changed on focus
    window.addEventListener("focus", () => {
      if (
        localStorage.getItem("lvlnumber") &&
        localStorage.getItem("lvlnumber") != "-1"
      ) {
        ...
        if (level.data != localStorage.getItem("lvlcode")) {
          ...
          util.confirm(
            "fileChangedConfirm",
            "The Level Was Modified Externally",
